...
class Runner:
    ...
    def load_x_train(self) -> pd.DataFrame:
        ...
        """
        # 学習データの読込を行う
        # dfs = [pd.read_feather(f'../features/{f}_train.feather') for f in self.features]
        # return pd.concat(dfs, axis=1, sort=False)
        return pd.read_feather('../features/All_train.feather')
    ...
    def load_x_test(self) -> pd.DataFrame:
        ...
        :return: テストデータの特徴量
        """
        # dfs = [pd.read_feather(f'../features/{f}_test.feather') for f in self.features]
        # return pd.concat(dfs, axis=1, sort=False)
        return pd.read_feather('../features/All_test.feather')
    ...
    def load_index_fold(self, i_fold: int) -> np.array:
        ...
        train_y = self.load_y_train()
        kf = KFold(n_splits=self.n_fold, random_state=6, shuffle=True)
